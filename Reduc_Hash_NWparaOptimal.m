function varargout = Reduc_Hash_NWMparaOptimal(varargin)
...
paralist={[1e-5,1e-3],[1e-7,1e-3],[1e-9,1e-3]};
for M=[2,3,4]
    ...
    for j=1:3
	num = num+1;
	if exist([dirname_write,'recRev_NWMulti_MparaOptimal_test_',int2str(num),'.mat'],'file') ~= 2
	...
	resRev.M = M;
        save([dirname_write,'recRev_NWMulti_MparaOptimal_test_',int2str(num),'.mat'],'resRev');
	end
    end
end
end
