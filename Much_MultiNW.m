function varargout = Much_MultiNW(varargin)
...
test_num=size(feature_test,1);%N*L
    batch = varargin{1}
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/Much_Multi/MultiNW_',batch,'/'];
...
for M = [1,2,3,4]
    for K = [8,16,24,32]
	filename = [dirname_write,'MultiNW_list500_',int2str(M),'_',int2str(K),'.mat'];
	...
	resRev.test_time = test_time(M,K/8);
	save([dirname_write,'MultiNW_list500_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
test_time
dlmwrite([dirname_write,'MuchNW_train_time.txt'],train_time);
dlmwrite([dirname_write,'MultiNW_test_time.txt'],test_time);
end
