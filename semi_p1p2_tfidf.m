...
function [test_time,train_time]=semi_p1p2_tfidf(varargin)
t1 = clock;
...
feature_data = load([dirname_load,'train_tfidf.txt']);
...
filename_train_Hc = [dirname_write,'semi_train_tfidf','_',int2str(M),'_',int2str(K),'.txt'];
filename_train_Hbinary = [dirname_write,'semi_train_tfidf_binary','_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_train_Hc,much_train_Y);
dlmwrite(filename_train_Hbinary,res.H');
test_data = load([dirname_load,'test_tfidf.txt'])'; %L*N
...
t3=clock;
filename_test_Hc = [dirname_write,'semi_test_tfidf_',int2str(M),'_',int2str(K),'.txt'];
filename_test_Hbinary = [dirname_write,'semi_test_tfidf_binary',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_test_Hc,much_test_Y);
dlmwrite(filename_test_Hbinary,test_hash_value'>=0);

test_time = etime(t3,t2);
%filename_Hc = [dirname_write,'semi_Hc_tfidf_',int2str(M),'_',int2str(K),'.txt']
%filename_H = [dirname_write,'semi_hash_binary__',int2str(M),'_',int2str(K),'.txt']
