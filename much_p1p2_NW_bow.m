...
function [test_time,train_time]=much_p1p2_NW_bow(varargin)
t1 = clock;
...
train_data=load([dirname_load,'train_NW_bow.txt'])';
M=varargin{1},
K=varargin{2},
paras.alpha = 2/K;
paras.delta = 1;
...
filename_W = [dirname_write,'much_train_NW_bow_W','_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_W,res.W);
filename_train_Hc = [dirname_write,'much_train_NW_bow','_',int2str(M),'_',int2str(K),'.txt'];
filename_train_Hbinary = [dirname_write,'much_train_NW_bow_binary','_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_train_Hc,much_train_Y);
dlmwrite(filename_train_Hbinary,res.H');
test_data = load([dirname_load,'test_NW_bow.txt'])';
...
t3=clock;
filename_test_Hc = [dirname_write,'much_test_NW_bow_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_test_Hc,much_test_Y);
filename_test_Hbinary = [dirname_write,'much_test_NW_bow_binary_',int2str(M),'_',int2str(K),'.txt'];
