...
test_time = ones(4,4);
train_time = ones(4,4);
feature_train = load([dirname_load,'train_bowMulti.txt']);
feature_test = load([dirname_load,'test_bowMulti.txt']);
train_num=size(feature_train,1);%N*L
test_num=size(feature_test,1);%N*L
for item =[0:1]
    batch = int2str(item)
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/SemiNTH_bowMulti_',batch,'/'];
...
for M = [2,3,1,4]
    for K = [8,16,24,32]
	filename = [dirname_write,'recRev_bowMulti_N2list500_',int2str(M),'_',int2str(K),'.mat'];
	if exist(filename,'file') == 0
        ...
        save([dirname_write,'recRev_bowMulti_N2list500_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
test_time
dlmwrite([dirname_write,'SemiNTH_bowMulti_train_time.txt'],train_time);
dlmwrite([dirname_write,'SemiNTH_bowMulti_test_time.txt'],test_time);
end
