function varargout = SemiNTH_bowNW_g1e5g2e9(varargin)
...
for term=[0:9]
    batch = int2str(term)
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/SemiNTH_bowNW_',batch,'/'];
...
for M = [2,3,1,4]
    for K = [8,16,24,32]
   	filename = [dirname_write,'recRev_bowNW_list200_g1e5g2e9_',int2str(M),'_',int2str(K),'.mat'];
	if exist(filename,'file') == 0
        ...
        save([dirname_write,'recRev_bowNW_list200_g1e5g2e9_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
test_time
dlmwrite([dirname_write,'SemiNTH_bowNW_M2e5e9_train_time.txt'],train_time);
dlmwrite([dirname_write,'SemiNTH_bowNW_M2e5e9_test_time.txt'],test_time);
end
