...
function [test_time,train_time]=much_p1p2_tfidf(varargin)
t1 = clock;
...
train_data = load([dirname_load,'train_tfidf.txt'])';
M=varargin{1},
K=varargin{2},
paras.alpha = 2/K;
paras.delta = 1;
...
filename_W = [dirname_write,'much_W_tfidf','_',int2str(M),'_',int2str(K),'.txt'];
filename_train_Hc = [dirname_write,'much_train_tfidf','_',int2str(M),'_',int2str(K),'.txt'];
filename_train_Hbinary = [dirname_write,'much_train_tfidf_binary','_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_W,res.W);
dlmwrite(filename_train_Hc,much_train_Y);
dlmwrite(filename_train_Hbinary,res.H');
test_data = load([dirname_load,'test_tfidf.txt'])';
...
t3=clock;
filename_test_Hc = [dirname_write,'much_test_tfidf_',int2str(M),'_',int2str(K),'.txt'];
filename_test_Hbinary = [dirname_write,'much_test_tfidf_binary_',int2str(M),'_',int2str(K),'.txt'];
