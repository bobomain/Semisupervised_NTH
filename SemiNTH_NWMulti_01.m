...
for item =[0:1]
    batch = int2str(item)
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/SemiNTH_NWMulti_',batch,'/'];
...
for M = [2,3,1,4]
    for K = [8,16,24,32]
	filename = [dirname_write,'recRev_NWMulti_list500_',int2str(M),'_',int2str(K),'.mat'];
	if exist(filename,'file') == 0
        ...
        save([dirname_write,'recRev_NWMulti_list500_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
