...
for paras.gamma=[1e-7,1e-9]
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/Reduc_Hash/NWtestparas/'];
    R =load([dirname_load,'NusWideMulti_CollectMatrix_0.txt']);
    ...
    for paras.gamma1=[1e-7,1e-3]
	for paras.gamma2 =[1e-3,1e-9]
	    num = num+1
	    paras.alpha = 2/K;
	    [test_B,train_B]=SemiStage_Pcatest(M,K,dirname_write,R,data,paras);
	    resRev=revolution_test(WTrue,test_B,train_B,M,K,rev_paras);
	    resRev.paras=paras;
	    save([dirname_write,'recRev_NWMulti_parastest_pca_',int2str(num),'.mat'],'resRev');
	end
    end
end
