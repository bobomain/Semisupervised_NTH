function varargout = Hash_regula_paraOptimal3M(varargin)
...
for M=[3]
    ...
    for j=1:9
	num = num+1;
	if exist([dirname_write,'hash_regula_3MparaOptimal_',int2str(num),'.mat'],'file') ~= 2
	paras.gamma1 = paralist{1,j}(1)
	paras.gamma2 = paralist{1,j}(2)
        paras.alpha = 2/K;
        [test_B,train_B]=SemiStage_regulaPcaOWtest(M,K,dirname_write,R,data,paras);
	...
	resRev.M = M;
        save([dirname_write,'hash_regula_3MparaOptimal_',int2str(num),'.mat'],'resRev');
	end
    end
end
end
