function varargout = Semi_NTH_lda_octave(varargin)
test_time = ones(4,4);
train_time = ones(4,4);
batch=varargin{1},
nneighbors=10;
...
for M = 1:4
    for K = [8,16,24,32]
	paras.alpha = 2/K;
        [test_time(M,K/8),train_time(M,K/8),test_B,train_B]=SemiNTH_lda(M,K,dirname_write,R_G,data,paras);
	resRev=revolution(WTrue,test_B,train_B,M,K,rev_paras);
	save([dirname_write,'SemiNTH_recRev_nei10_',int2str(M),'_',int2str(K),'.mat'],'resRev');
end
end
train_time
test_time
