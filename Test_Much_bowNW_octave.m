...
batch =1, 
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/much_p1p2_NW_bow_Hc_',int2str(batch),'/'];
    R =load([dirname_load,'NusWide_CollectMatrix_',int2str(batch),'.txt']);
    for M = 1:4
        for K = [8,16,24,32]
	    test_H = load([dirname_write,'much_test_NW_bow_binary_',int2str(M),'_',int2str(K),'.txt']);
	    train_H = load([dirname_write,'much_train_NW_bow_binary_',int2str(M),'_',int2str(K),'.txt']);
	    [I,J]=size(train_H);
            if I<J
                train_H = train_H';
            end
	    ...
	    resRev=revolution(WTrue,test_B,train_B,M,K,rev_paras);
            save([result_file,'much_NWbow_recRev_',int2str(batch),'_',int2str(M),'_',int2str(K),'.mat'],'resRev');
        end
    end
