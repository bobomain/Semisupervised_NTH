function varargout = SemiNTH_NW_g1e5g2e9(varargin)
...
for item =[0:9]
    batch = int2str(item)
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/SemiNTH_NW_',batch,'/'];
...
for M = [2,3,1,4]
    for K = [8,16,24,32]
	filename = [dirname_write,'recRev_list200_g1e5g2e9_',int2str(M),'_',int2str(K),'.mat'];
	if exist(filename,'file') == 0
        ...
        save([dirname_write,'recRev_list200_g1e5g2e9_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
test_time
dlmwrite([dirname_write,'SemiNTH_M2e5e9_train_time.txt'],train_time);
dlmwrite([dirname_write,'SemiNTH_M2e5e9_test_time.txt'],test_time);
end
