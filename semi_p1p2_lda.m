...
function [test_time,train_time]=semi_p1p2_lda(varargin)
t1 = clock;
dirname_write = varargin{3};
dirname_load = '/home/lf/much_code/monog_file/'
R =load([dirname_load,'dblp_CollectMatrix_',int2str(varargin{4}),'.txt']);
feature_data = load([dirname_load,'train_lda.txt']);
train_num=size(feature_data,1),
%%pre-proceding
nneighbors=30;
train_data = feature_data'; %L*N
...
filename_train_Hc = [dirname_write,'semi_train_lda','_',int2str(M),'_',int2str(K),'.txt'];
filename_train_Hbinary = [dirname_write,'semi_train_lda_binary','_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_train_Hc,much_train_Y);
dlmwrite(filename_train_Hbinary,res.H');
test_data = load([dirname_load,'test_lda.txt'])'; %L*N
...
t3=clock;
filename_test_Hc = [dirname_write,'semi_test_lda_',int2str(M),'_',int2str(K),'.txt'];
filename_test_Hbinary = [dirname_write,'semi_test_lda_binary_',int2str(M),'_',int2str(K),'.txt'];
dlmwrite(filename_test_Hc,much_test_Y);
dlmwrite(filename_test_Hbinary,test_hash_value'>=0);

test_time = etime(t3,t2);
%filename_Hc = [dirname_write,'semi_Hc_lda_',int2str(M),'_',int2str(K),'.txt']
%filename_H = [dirname_write,'semi_hash_binary_lda_',int2str(M),'_',int2str(K),'.txt']
%filename_muchW = [dirname_write,'semi_muchW_lda_',int2str(M),'_',int2str(K),'.txt']
%filename_sshW = [dirname_write,'semi_sshW_lda_',int2str(M),'_',int2str(K),'.txt']
%filename_B = [dirname_write,'semi_B_lda_',int2str(M),'_',int2str(K),'.txt']
