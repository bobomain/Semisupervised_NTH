...
for item =[0:1]
    batch = int2str(item)
    dirname_write = ['/home/lf/much_code/dblp_mongo_vector/SemiNTH_N2Multilda_',batch,'/'];
...
for M = [2,3,1,4]
    for K = [8,16,24,32]
	filename = [dirname_write,'recRev_ldaMulti_N2list500_',int2str(M),'_',int2str(K),'.mat'];
	if exist(filename,'file') == 0
        ...
        save([dirname_write,'recRev_ldaMulti_N2list500_',int2str(M),'_',int2str(K),'.mat'],'resRev');
	end
end
end
train_time
test_time
dlmwrite([dirname_write,'SemiNTH_ldaMulti_train_time.txt'],train_time);
dlmwrite([dirname_write,'SemiNTH_ldaMulti_test_time.txt'],test_time);
end
